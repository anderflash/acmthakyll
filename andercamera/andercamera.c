        ...
        unsigned int i;
				...
				if(!jump)
				{
					...
					case IO_METHOD_READ:
									...
									*p = camera->buffers[0].start;
									*size = camera->buffers[0].length;
					...
					case IO_METHOD_MMAP:
									...
									*p = camera->buffers[buf.index].start;
									*size = buf.bytesused;
...
static void init_read(Camera *camera, unsigned int buffer_size)
{
        camera->buffers = calloc(1, sizeof(*(camera->buffers)));
...
static void init_mmap(Camera *camera)
{
        ...
        for (camera->n_buffers = 0; camera->n_buffers < req.count; ++camera->n_buffers) {
                ...
                camera->buffers[camera->n_buffers].start =
...
static void init_userp(Camera *camera, unsigned int buffer_size)
{
        struct v4l2_requestbuffers req;
        ...
        camera->buffers = calloc(4, sizeof(*(camera->buffers)));
        ...
        for (camera->n_buffers = 0; camera->n_buffers < 4; ++camera->n_buffers) {
                ...
                camera->buffers[camera->n_buffers].start = malloc(buffer_size);
...
static void ANDERopen_device(Camera *camera)
{
        ...
        camera->fd = open(dev_name, O_RDWR /* required */ | O_NONBLOCK, 0);
